...
const LogoImg = ({style, size=17}) => (
  ...
  <img src={(typeof window!=="undefined"&&window.__ANDALUS_LOGO_B64)?window.__ANDALUS_LOGO_B64:"/andalus-logo.jpg"} alt="شركة الأندلس التعليمية" style={{height:size*2.4,width:"auto",maxWidth:"100%",objectFit:"contain"}}
   onError={(e)=>{ e.target.style.display="none"; if(e.target.nextSibling) e.target.nextSibling.style.display="block"; }}/>
  <div style={{display:"none",fontSize:size,fontWeight:900,color:"#1D5A8A",letterSpacing:0.3,lineHeight:1.2,textAlign:"center",whiteSpace:"nowrap"}}>شركة الأندلس التعليمية</div>
  </div>
);
...
const getEvalModel = (roleOrUser) => {
  ...
  // المتابع الفني نوعان: الوكيل-كمتابع (deputy_role) يُقيَّم كالقيادي، وغيره (المشرف المختص) كالمعلم
  // الافتراضي = المشرف المختص (الأشيع)، فلا يُعامَل كوكيل إلا إن حُدّد نوعه deputy_role صراحةً
  if (role === "supervisor") {
    return subtype === "deputy_role" ? "leader" : "employee";
  }
...
const PER_PERSON_BUDGET = { teacher:400, specialist:400, deputy:1500, stage_mgr:1500, branch_ext:1000 };
const personBudget = (u)=>{
  if(!u) return 0;
  if(u.role==="employee") return 400;                         // معلم/إداري
  if(u.role==="supervisor"&&u.roleSubtype==="specialist") return 400; // مشرف مختص
  if(u.role==="deputy"||u.role==="stage_mgr") return 1500;    // وكيل/مدير مرحلة
  if(u.role==="branch_ext") return 1000;                       // امتداد فني
  return 0;
};
// هل يخضع الشخص للاعتماد المالي عبر مدير المرحلة؟ (معلم/إداري + مشرف مختص فقط — لا القيادات ولا المتابع الفني)
const underStageFinance = (u)=> u && (u.role==="employee");   // مدير المرحلة يعتمد ماليّاً لموظفي مرحلته (معلم/إداري)
// سقف مدير المرحلة = 400 × عدد موظفيه (معلم/إداري) في مرحلته
const stageMgrCap = (mgr, allUsers)=>{
  const emps=(allUsers||[]).filter(u=>u.role==="employee"&&u.branch===mgr.branch&&u.stage===mgr.stage);
  return emps.length*400;
};
// سقف مدير الفرع = مجموع معاملات كل التابعين لفرعه
const branchMgrCap = (mgr, allUsers)=>{
  const branches = (mgr.branches&&mgr.branches.length)?mgr.branches:(mgr.branch?[mgr.branch]:[]);
  return (allUsers||[]).filter(u=>branches.includes(u.branch)).reduce((s,u)=>s+personBudget(u),0);
};
...
const PLAN_APPROVAL = {
  ...
  // احتياط لمن بلا نوع فرعي: الافتراضي = المشرف المختص → يعتمده المشرف التعليمي (لا مدير المرحلة)
...
const PLAN_APPROVER_LABEL = {
  "deputy":"مدير المرحلة (اعتماد فني) ثم مدير الفرع", "supervisor":"المشرف التعليمي (الامتداد الفني التعليمي)",
...
const SCORE_LABELS = ["","دون التوقعات","جيد","جيد جداً","ممتاز","يفوق التوقعات"];
const SCORE_COLORS = ["","#EF4444","#F97316","#F59E0B","#3B82F6","#10B981"];

const getLevel = s => {
  if (s >= 4.5) return { label:"يفوق التوقعات", color:"#10B981" };
  if (s >= 3.5) return { label:"ممتاز",         color:"#3B82F6" };
  if (s >= 2.5) return { label:"جيد جداً",       color:"#F59E0B" };
  if (s >= 1.5) return { label:"جيد",            color:"#F97316" };
  return             { label:"دون التوقعات",    color:"#EF4444" };
};
...
// المفاتيح المستخرجة تلقائيّاً من نصوص النظام (لتغذية الترجمة)
const I18N_KEYS = [
  "(لا يدخل المتوسط)",
  "(للإدارة فقط — لا يظهر للموظف أو الزملاء)",
  "(لمدير النظام فقط)",
  "+ إضافة",
  "+ إضافة الشهادة",
  "+ إضافة بند",
  ...
  "-- بدون --",
  "4 أو 5 أرقام",
  "، وتظهر كمتوسط دون كشف هوية المُقيّم.",
  "أخصائي إدارة وظيفية",
  "أخصائي التدريب",
  "أدخل بريدك (اسم المستخدم) ليصلك رابط إعادة التعيين",
  "أساسية",
  "أضف جدارات أولاً من تبويب مصفوفة الجدارات",
  "أضف مصادر أولاً من تبويب المصادر",
  "إدارة الترجمات",
  "إضافة",
  "إظهار كلمات المرور",
  ...
  "اختيار ←",
  "ارتباطاً جديداً عبر",
  "استيراد الحسابات دفعةً.",
  "اسم المستخدم",
  "اطّلاع منظّم على خطة أي موظف حسب الفرع والمرحلة",
  "الأخصائيين",
  "الأسلوب",
  "الإنجاز",
  "الاسم",
  "الاسم، النوع، الجهة، الرابط، الساعات، التكلفة",
  "الامتدادات الفنية",
  "البريد الفعلي للموظف",
  "البند المراد استبداله",
  "التخطيط الاستراتيجي، كتاب الإدارة الحديثة؛ دورة القيادة",
  "التقييم الذاتي",
  "التكلفة",
  "التكلفة (ريال)",
  ...
  "المتوسط العام",
  "المتوسّط من 5",
  "المتوسّط من 5 — اكتمال التقييم يظهر بعدد الأشخاص",
  "المجموع",
  "المدرب",
  "المدير المباشر",
  "المرؤوسون",
  ...
  "النوع",
  "النوع الفرعي",
  "امتداد فني لإدارة وظيفية",
  "انتهت المدة",
  "بانتظار الاعتماد",
  "بانتظار الفني",
  "بتاريخ",
  "بصفتك متابعهم الفني.",
  "بصفتك مديرهم المباشر، و",
  "بعد القفل",
  "تأكيد الحذف",
  "تأكيد قفل التقييم",
  ...
  "ترتيب الجدارات",
  "ترتيب الموظفين",
  "تسجيل قراءة النتائج مع الموظف",
  "تظهر كمتوسط دون كشف الهوية.",
  "تعديل",
  "تقييمك كموظف من الأطراف المختلفة",
  "تقييمك من الأطراف حسب نموذج وظيفتك",
  "تكلفة الخطة",
  "تم",
  "تم التنفيذ",
  "تم تأكيد التقييم وقفله",
  "تمت قراءة النتائج",
  "جاري",
  "جاري التحميل...",
  "جاري التنفيذ",
  ...
  "ساعات تدريبية",
  "سبب طلب التعديل",
  "ستُضاف مصادر هذه الجدارة قريباً",
  "سجّل دخولك لمتابعة ملفك المهني",
  "سرّي",
  "سيُحذف حساب",
  ...
  "عرض",
  "غير معتمدة",
  "فقط — قيّم كل بند من 1 إلى 5",
  "فنية",
  "قسم التدريب • الموارد البشرية",
  "قيّم نفسك في جداراتك (أساسية + عامة + فنية)",
  "قيّمك",
  "كلمة المرور",
  "كلمة المرور الجديدة",
  "كلمة المرور الحالية",
  ...
  "لا تقييم بعد",
  "لا توجد بنود بعد",
  "لا توجد جدارات ضمن نطاق تقييمك لهذا الموظف",
  "لا توجد دورات حضورية داخلية مختارة بعد",
  "لا توجد فجوات حرجة أو لم يكتمل التقييم",
  "لا توجد نتائج",
  "لا جدارة تطابق البحث",
  "لا حسابات مطابقة",
  "لا خطة بعد",
  "لا خطط في هذه المرحلة بعد",
  "لا شهادات — أضِف أدناه.",
  "لا شواهد مرفقة بعد",
  "لا طلبات بعد.",
  "لا فروع بها موظفون بعد",
  "لا مصادر مطابقة — أضِفها من مكتبة المصادر أولاً",
  "لا مصطلحات مطابقة",
  "لا معلمون مرتبطون بهذا المشرف بعد.",
  "لا موظفون في هذه المرحلة",
  "لا يوجد بيانات في هذا النطاق",
  "لا يوجد مشرفون مختصون في فرعك بعد.",
  "لا يوجد موظفون مرتبطون بك",
  "لا يوجد موظفون مرتبطون بك فنياً",
  "لديهم بيانات",
  "لم تُحدَّد في المكتبة — أدخلها لإكمال الخطة",
  "لم يبدأ",
  "لم يتم",
  "لم يتم تسجيل أي قراءة للنتائج بعد",
  "لم يضع الموظف خطته بعد",
  "لم يكتمل أي تقييم بعد",
  "لم يُضَف بند لهذه الفئة بعد (بند واحد كحد أقصى)",
  "لم يُقيَّم",
  "لم يُقيّم",
  "لن تتمكن من التعديل",
  "متابعة التطور المهني",
  "متابعة التطور المهني متاحة للمتابع الفني فقط",
  "متابعة تقييم الأداء",
  "متوسط التقييمين",
  ...
  "مسودّة",
  "مصفوفة الجدارات",
  "مصفوفة كائنات بنفس الحقول",
  "معتمدة",
  "معتمدة من الفني",
  "معتمدة من المتابع الفني",
  "مغلق",
  "مفتوح",
  "مكتبة المصادر",
  "ملف التطور المهني",
  ...
  "من",
  "منصة التطور",
  "نتائج تقييم معتمدة من مدير الفرع",
  "نتيجة تقييم الأداء معتمدة من مدير الفرع",
  "نسب الإنجاز",
  "نسيت كلمة السر؟",
  "نسيت كلمة السر؟ 🔑",
  "نهائياً",
  "نوع المتابع الفني",
  "وبمعدل",
  "وذلك ضمن برامج التدريب الداخلي بشركة الأندلس التعليمية",
  "وكيل",
  "يرجى التواصل مع قسم التدريب - الموارد البشرية",
  "يعرض هذا التحليل موظفيك ومتوسط أدائهم لكل جدارة",
  "يفوق",
  "يفوق التوقعات",
  "— اختر الجدارة —",
  "— اختر المرحلة —",
  ...
  "— اختر —",
  "— بدون شهادة احترافية —",
  "• الحدّ الأدنى 15 ساعة.",
  "ℹ️ ستظهر «حالة التنفيذ» بعد اعتماد خطتك.",
  "← العودة لتسجيل الدخول",
  "← وزن",
  "↩ إلغاء",
  "↩ إلغاء الاعتماد",
  "↩️ إعادة التخطيط للموظف",
  "↺ إعادة الضبط",
  "↺ الافتراضية",
  "⌛ انتهت مدة التقييم",
  "⏱️ إجمالي الساعات",
  ...
  "⏱️ عدد الساعات",
  "⏳ بانتظار",
  "⏳ بانتظار إغلاق الموظف للتخطيط",
  "⏳ طلب تعديل معلّق",
  "⏳ في الانتظار",
  "⏳ لم يكتمل",
  "⏳ يُنشَّط قياس الأثر بعد إتمام تنفيذ الدورة",
  "⏳ يُنشَّط قياس الأثر بعد اعتماد المتابع الفني للخطة",
  "⏳ يُنشَّط قياس الأثر بعد اعتماد مدير الفرع للمرحلة",
  "◆ حالة تقييمي الشامل 360°",
  "⚖️ مصفوفة الأوزان — قابلة للتعديل (اضغط للفتح)",
  "⚠️ بيانات الدخول غير صحيحة",
  "⚠️ رقم الهوية يجب أن يكون 10 أرقام",
  "⚡ اختيار من المكتبة",
  "✅ أسلوب التقييم",
  "✅ أسلوب التقييم المقترح",
  "✅ أُنشئ",
  "✅ اعتماد",
  "✅ اعتماد الخطة",
  "✅ اعتماد الخطط والتعديلات",
  "✅ اعتماد وإنشاء",
  "✅ الخطط معتمدة",
  "✅ حفظ وإغلاق التخطيط",
  "✅ موافقة",
  "✍️ رسالة تظهر للمستخدمين وقت الصيانة",
  "✏️ أخرى (أكتبها بنفسي)...",
  "✏️ إدخال يدوي",
  "✏️ إعادة تخطيط البند",
  "✏️ اسم الشهادة",
  "✏️ تعديل",
  "✏️ طلب تعديل بند (بديل)",
  "✏️ طلب تعديل بند (تخطيط بديل)",
  "✓ تعديل معتمد",
  "✓ تم",
  "✓ تم التقييم",
  "✓ حُفظ",
  ...
  "➕ إضافة مصدر",
  "➕ طلبات الحسابات",
  "⬆️ تقييم الرؤساء المباشرين",
  "⬆️ رفع ملف",
  "⭐ ممتاز (5)",
  "🇸🇦 المصطلح العربي",
  "🌐 إدارة الترجمة الإنجليزية",
  "🎓 أسلوب التدريب",
  "🎓 الدورات",
  "🎓 تنزيل شهادة الحضور",
  "🎓 دورة",
  ...
  "🎖️ الشهادة",
  "🎯 التطور المهني",
  "🎯 التقرير التنفيذي — التطور المهني",
  "🎯 تقييم مقدّمي الخدمة",
  "🎯 توزيع البنود على الفئات",
  "🎯 خطة التطور المهني",
  "🎯 خطتي المهنية",
  "🎯 خطتي وتقييمي",
  "🎯 متابعة التطور المهني",
  ...
  "🏆 الكلية",
  "🏛️ الجهة",
  "🏛️ الجهة التي ستقدّم البرنامج",
  "🏛️ الجهة المنفّذة",
  "🏛️ القيادات التابعون لك مباشرة",
  "🏛️ ترتيب الفروع بمتوسط الأداء",
  "🏛️ ترتيب الفروع بنسبة التنفيذ",
  "🏛️ جميع الفروع",
  "🏛️ قيادات الفرع (مدراء المراحل والوكلاء)",
  "🏛️ قيادات الفرع — متابعة التطور المهني",
  "🏛️ كل الفروع",
  "🏢 جميع",
  "🏫 إدارة الدورات الحضورية الداخلية",
  "🏫 تفاصيل الدورة الحضورية من إدارة التدريب",
  "🏫 دورة داخلية",
  "🏫 فرع",
  "👁️ عرض الخطة",
  "👁️ عرض الخطة كاملة",
  ...
  "👥 كل الأدوار",
  "👥 متابعة التطور المهني",
  "👥 متابعة المشرفين المختصين",
  "👥 متابعة فريقي",
  "👨‍🏫 اسم المدرب",
  "💡 أنت تُقيّم الجدارات",
  "💡 اسم المستخدم =",
  "💡 تقيّم",
  "💡 قيّم كل بند من 1 إلى 5. تقييمك",
  "💡 يُفضّل أن تنتهي فترة التنفيذ في الفصل الدراسي الأول",
  "💰 إجمالي التكلفة",
  "💰 الاعتماد المالي",
  "💰 الاعتماد المالي النهائي",
  "💰 التكلفة",
  "💰 سقفك المالي للاعتماد",
  "💼 المسميات الوظيفية",
  "💼 المسمّى الوظيفي",
  "💾 حفظ",
  "💾 حفظ الأوزان",
  ...
  "📅 تاريخ التنفيذ المتوقّع",
  "📈 ترتيب الجدارات",
  "📈 توزيع التقديرات",
  "📈 حالة تنفيذ البنود",
  "📊 التقرير التنفيذي",
  "📊 التقرير التنفيذي — تقييم الأداء",
  "📊 تفصيل النتائج",
  "📊 تقييم أدائي",
  "📊 تقييم أدائي الوظيفي",
  "📊 تقييم الأداء",
  ...
  "📊 حالة التنفيذ",
  "📊 عرض بطاقتي الكاملة",
  "📊 لوحة متابعة تنفيذ خطتي",
  "📊 متابعة تقييم الأداء",
  "📊 مدة الحضور (%)",
  "📊 ملخّص التطور المهني حسب المرحلة",
  "📊 ملخّص تقييم الأداء حسب المرحلة",
  "📊 نسبة التنفيذ",
  "📋 أسلوب قياس الأثر",
  "📋 استعراض خطط التطور المهني",
  "📋 استعراض خطط التطوّر المهني (حسب الفرع)",
  "📋 الجدارات والبنود",
  "📋 المصادر",
  "📋 بيانات المصدر (تلقائية من المكتبة)",
  "📋 تفاصيل تقييم الموظفين حسب الفرع (اضغط للعرض)",
  "📋 تقييم فريقي",
  "📋 حالة تقييم الأداء لكل مرحلة",
  "📋 خطط معتمدة",
  "📋 خطط معتمدة فنياً",
  "📋 خطط وتقييمات المشرفين المختصين أنفسهم (اضغط للعرض)",
  "📋 عدد البنود",
  "📋 عرض بطاقتي الكاملة",
  "📋 متابعة تقييم الأداء",
  "📋 وكيل (للإداريين)",
  "📌 مصدر الاحتياج",
  "📍 حالة تنفيذ الدورة",
  "📍 مكان التنفيذ (قاعة أو رابط اجتماع)",
  "📎 إرفاق ملف (حتى 1MB)",
  "📎 الشواهد",
  "📎 ملف",
  "📏 أسلوب غير معروف",
  "📏 قياس الأثر",
  "📏 لم يُحدَّد أسلوب قياس الأثر لهذا البند",
  "📑 التقرير المجمع — موظفوك",
  "📑 تحليل التقييمات",
  "📑 تحليل تقييمات موظفي الفرع (اضغط للعرض)",
  "📑 تحليل تقييمات موظفي مرحلتي (اضغط للعرض)",
  "📖 إدارة مكتبة المصادر التدريبية",
  "📖 الكتب",
  "📖 كتاب",
  "📖 من مكتبة المصادر",
  "📚 اسم البرنامج أو الكتاب",
  "📚 اسم البرنامج أو المادة التعليمية أو الكتاب",
  "📚 المرحلة",
  "📚 المصدر المقترح",
  "📚 كل المراحل",
  "📚 مرحلة",
  "📝 ملاحظة المتابع الفني",
  "📝 ملاحظتك للموظف على هذا البند (تصله ليراعيها)",
  "📤 إرسال الطلب لمدير الفرع",
  "📥 استيراد",
  "📥 استيراد الحسابات",
  "📥 استيراد الربط جماعياً",
  "📥 استيراد ربط المصادر بالجدارات",
  "📥 استيراد ومعاينة",
  "📧 البريد الإلكتروني",
  "📨 إرسال الطلب لمدير النظام",
  "🔁 التقييم الثاني",
  "🔁 نتائج التقييمين — الدرجة النهائية = متوسطهما",
  "🔁 يُقيَّم مرتين",
  "🔄 حالة التنفيذ",
  "🔄 نسب اكتمال التقييم لكل طرف",
  "🔍 المتابع الفني",
  "🔍 متابع فني",
  "🔍 مشرف مختص (للمعلمين)",
  "🔑 تغيير كلمة المرور",
  ...
  "🔑 كلمة المرور",
  "🔒 إغلاق التقييم الثاني",
  "🔒 إغلاق الكل",
  "🔒 التقييم الذاتي مقفول",
  "🔒 التقييم مغلق",
  "🔒 التقييم مقفول — التعديل محظور",
  "🔒 تأكيد القفل",
  "🔒 تفصيل تقييمات زملاء التخصص",
  "🔒 حفظ نهائي وقفل",
  "🔒 مغلق",
  "🔒 مقفول",
  "🔓 بدء التقييم الثاني",
  "🔓 فتح الكل",
  "🔗 الامتدادات الفنية في الفروع",
  "🔗 الرابط",
  "🔗 الربط بالجدارات",
  "🔗 تفاصيل الشهادة",
  "🔗 رابط",
  ...
  "🖨️ طباعة",
  "🖼️ صورة",
  "🗂️ مصفوفة الجدارات والمسميات الوظيفية",
  "🗑 حذف",
  "🗓️ نوافذ التقييم لكل فرع",
  "🛠️ وضع الصيانة (تعطيل الدخول)",
  "🤝 تحديد زملاء التخصص المُقيِّمين",
  "🤝 تقييم الزملاء (الجدارات الأساسية فقط — 5%)",
  "🤝 زميل",
  "🪪 رقم الهوية",
];
...
// ═══ الترجمة التلقائية الشاملة ═══
// نعترض React.createElement لتمرير كل نصّ عربي ظاهر عبر t() تلقائيّاً.
// هذا يترجم كل نصوص النظام دون ربط يدويّ، ويبقى آمناً: عند العربية أو غياب ترجمة يُعاد النصّ كما هو.
(function installAutoTranslate(){
  if (typeof window==="undefined" || typeof React==="undefined" || React.__i18nPatched) return;
  const AR = /[\u0600-\u06FF]/;   // يحوي حرفاً عربيّاً
  const translateChild = (ch) => {
    // نترجم فقط السلاسل التي تحوي عربيّة؛ نترك الأرقام والعناصر كما هي
    if (typeof ch === "string") {
      if (window.__I18N && window.__I18N.lang==="en" && AR.test(ch)) {
        const trimmed = ch.trim();
        if (!trimmed) return ch;
        const out = t(trimmed);
        // نحافظ على المسافات المحيطة إن كانت الترجمة مختلفة
        if (out !== trimmed) return ch.replace(trimmed, out);
      }
      return ch;
    }
    return ch;
  ...
  React.createElement = function(type, props, ...children){
    if (window.__I18N && window.__I18N.lang==="en" && children && children.length){
      children = children.map(c=>Array.isArray(c)?c.map(translateChild):translateChild(c));
    }
...
function generateAttendanceCertificate({ name, courseName, date, hours, trainer }) {
  ...
  <div class="cert">
    <div class="logo"><img src="${window.location.origin}/andalus-logo.jpg" alt="شركة الأندلس التعليمية" style="height:70px;width:auto;object-fit:contain;" onerror="this.style.display='none';this.nextElementSibling.style.display='block';"/><span style="display:none;">شركة الأندلس التعليمية</span></div>
...
function groupStats(empList, evals) {
  ...
  const levelDist={"يفوق التوقعات":0,"ممتاز":0,"جيد جداً":0,"جيد":0,"دون التوقعات":0};
...
function LoginScreen({ onLogin }) {
  ...
  // وضع الصيانة: يمنع دخول غير مدير النظام
  const maint = await st.getShared("maintenanceMode_360c").catch(()=>null);
  if (maint && maint.enabled) { setErr(false); setMaintMsg(maint.message||"النظام قيد التحديث، يُرجى المحاولة لاحقاً"); return; }
  ...
  {maintMsg&&<div style={{color:"#B45309",fontSize:13,marginBottom:12,background:"#F59E0B12",borderRadius:14,padding:"12px 16px",border:"1px solid #F59E0B40",fontWeight:700,textAlign:"center",lineHeight:1.7}}>🛠️ {maintMsg}</div>}
...
function EvalForm({ partyKey, targetUser, existingScores, onSave, onCancel, locks, onLock, lockKeyOverride }) {
  ...
  const [witnessFiles,setWitnessFiles] = useState(existingScores?.__witnessFiles||{}); // مرفقات الشاهد (base64، حد 1MB)
  const [fileErr,setFileErr] = useState({});
  const requiresWitness = true; // الشاهد مطلوب لكل أطراف التقييم بلا استثناء عند الدرجة 5
  ...
  if (updated[idx] !== 5 && requiresWitness) {
   ...
   setWitnessFiles(pf=>{ const nf={...pf}; delete nf[`${comp}__${idx}`]; return nf; });
  }
  ...
  const onWitnessFile = (comp,idx,file) => {
   const key=`${comp}__${idx}`;
   setFileErr(pe=>{ const ne={...pe}; delete ne[key]; return ne; });
   if(!file) return;
   if(file.size > 1024*1024){ setFileErr(pe=>({...pe,[key]:"حجم المرفق يتجاوز 1 ميجابايت"})); return; }
   const reader=new FileReader();
   reader.onload=()=>{ setWitnessFiles(pf=>({...pf,[key]:{name:file.name,type:file.type,size:file.size,data:reader.result}})); };
   reader.onerror=()=>setFileErr(pe=>({...pe,[key]:"تعذّرت قراءة الملف"}));
   reader.readAsDataURL(file);
  };
  const removeWitnessFile = (comp,idx)=>setWitnessFiles(pf=>{ const nf={...pf}; delete nf[`${comp}__${idx}`]; return nf; });

  // تحقّق: كل بند بالدرجة 5 يحتاج شاهداً صالحاً (رابط، أو نصّ 50-150 حرف، أو مرفق)
  const missingWitness = (()=>{
   for(const c of comps){
    const items=getActiveComps()[c]?.items||[];
    for(let i=0;i<items.length;i++){
     if((scores[c]?.[i]||0)===5){
      const w=(witnesses[`${c}__${i}`]||"").trim();
      const hasFile=!!witnessFiles[`${c}__${i}`];
      const isLink=/^https?:\/\//i.test(w);
      if(!(isLink||(w.length>=50&&w.length<=150)||hasFile)) return true;
     }
    }
   }
   return false;
  ...
  {v}
   ...
   {/* حقل الشاهد عند الدرجة 5 — رابط أو توضيح لفظي 50-150 حرف أو مرفق ≤1MB */}
   {needsWitness&&(()=>{
   const wlen=witnessText.trim().length;
   const isLink=/^https?:\/\//i.test(witnessText.trim());
   const wKey=`${c}__${idx}`;
   const wFile=witnessFiles[wKey];
   const validW = isLink || (wlen>=50 && wlen<=150) || !!wFile;
   return (
   <div style={{marginTop:8,padding:"10px 12px",background:"#10B98108",border:`1px solid ${validW?"#10B98140":"#F59E0B40"}`,borderRadius:8}}>
   <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:6,marginBottom:6,flexWrap:"wrap"}}>
   <div style={{display:"flex",alignItems:"center",gap:6}}>
   <span style={{fontSize:14}}>📎</span>
   <span style={{fontSize:11,color:validW?"#10B981":"#F59E0B",fontWeight:700}}>
   {validW?"✓ الشاهد مكتمل":"مطلوب: شاهد على الدرجة الكاملة (رابط، توضيح 50–150 حرف، أو مرفق)"}
   </span>
   </div>
   {!isLink&&witnessText.trim().length>0&&(
   <span style={{fontSize:10,fontWeight:700,color:wlen<50?"#F59E0B":wlen>150?"#EF4444":"#10B981"}}>{wlen}/150</span>
   )}
   </div>
   <textarea
   value={witnessText}
   onChange={e=>{ if(isLocked)return; let v=e.target.value; if(!/^https?:\/\//i.test(v.trim())) v=v.slice(0,150); setWitness(c,idx,v); }}
   readOnly={isLocked}
   rows={2}
   placeholder="أدرج رابطاً (يبدأ بـ http) أو توضيحاً لفظياً بين 50 و150 حرفاً، أو أرفق ملفاً بالأسفل"
   style={{width:"100%",padding:"8px 10px",background:"#FFFFFF",border:`1px solid ${validW?"#10B98130":"#F59E0B30"}`,borderRadius:7,color:"#1E293B",fontSize:11,boxSizing:"border-box",outline:"none",resize:"vertical",fontFamily:"inherit"}}
   />
   {!isLink&&wlen>0&&wlen<50&&!wFile&&<div style={{fontSize:9,color:"#F59E0B",marginTop:3}}>⚠️ التوضيح قصير — أضف {50-wlen} حرفاً على الأقل (أو أدرج رابطاً أو مرفقاً).</div>}
   {/* المرفق */}
   {!isLocked&&(
   <div style={{marginTop:8,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
   {!wFile?(
   <label style={{fontSize:10,color:"#2E7FB8",fontWeight:700,cursor:"pointer",background:"#2E7FB812",border:"1px dashed #2E7FB840",borderRadius:8,padding:"6px 12px"}}>
   📎 إرفاق ملف (حتى 1MB)
   <input type="file" onChange={e=>onWitnessFile(c,idx,e.target.files?.[0])} style={{display:"none"}}/>
   </label>
   ):(
   <div style={{display:"flex",alignItems:"center",gap:8,background:"#10B98112",borderRadius:8,padding:"5px 10px"}}>
   <span style={{fontSize:10,color:"#059669",fontWeight:700}}>📎 {wFile.name} ({Math.round(wFile.size/1024)}KB)</span>
   <button onClick={()=>removeWitnessFile(c,idx)} style={{background:"none",border:"none",color:"#EF4444",fontSize:12,cursor:"pointer"}}>✕</button>
   </div>
   )}
   {fileErr[wKey]&&<span style={{fontSize:10,color:"#EF4444",fontWeight:700}}>⚠️ {fileErr[wKey]}</span>}
   </div>
   )}
   {isLocked&&wFile&&<div style={{fontSize:10,color:"#059669",marginTop:6}}>📎 مرفق: {wFile.name}</div>}
   </div>
   );
   })()}
   </div>
...
function CompetenciesEditor({ comps, jobs, onSaveComps, onSaveJobs, onReset, roleItems, onSaveRoleItems }) {
  ...
  {(comp.items||[]).map((item,idx)=>(
   <div key={idx} style={{display:"flex",gap:5,alignItems:"center",marginBottom:7,marginTop:idx===0?10:0}}>
   <span style={{width:22,fontSize:11,color:"#5B7A9E",flexShrink:0,textAlign:"center"}}>{idx+1}</span>
   <input value={item} onChange={e=>updateItem(cn,idx,e.target.value)} style={{flex:1,minWidth:0,padding:"9px 12px",background:"#F4F9FE",border:"1px solid #C7DBF0",borderRadius:8,color:"#1E293B",fontSize:12.5,lineHeight:1.5}}/>
...
function Card360({ targetUser, empEval, onSaveIdp, idpData, onClose, readings, onSaveReadings, currentUser, hidePrint, allEvals, allUsers, impactData = {}, approvals = {} }) {
  ...
  {/* الشهادة الاحترافية جزء من خطة التطوّر — تُعرض هناك لا في بطاقة التقييم (تفادياً للخلط) */}
  ...
  <div style={{display:"flex",gap:6}}>
   ...
   {isValidUrl(F("url"))&&<a href={F("url")} target="_blank" rel="noopener noreferrer" style={{padding:"8px 10px",borderRadius:8,background:"#3B82F615",color:"#3B82F6",fontSize:12,textDecoration:"none"}}>🔗</a>}
  </div>
  </div>
  <div>
...
function RequestAccountForm({ user, onSubmit }) {
  ...
  const [f,setF] = useState({ name:"", username:"", nationalId:"", jobNumber:"", password:"", role:"employee", roleSubtype:"", job:"", stage:"", branch: myBranches[0]||"" });
  ...
  <div style={{background:"#FFFFFF",border:"1px solid #EC489925",borderRadius:14,padding:18}}>
   ...
   <div><label style={lS}>🔢 الرقم الوظيفي</label><input value={f.jobNumber||""} onChange={e=>set("jobNumber",e.target.value.replace(/\D/g,"").slice(0,6))} inputMode="numeric" placeholder="4 أو 5 أرقام" style={iS}/></div>
   ...
   <button onClick={()=>{ if(canSubmit){ onSubmit({...f,username:cleanEmail}); setF({name:"",username:"",nationalId:"",jobNumber:"",password:"",role:"employee",roleSubtype:"",job:"",stage:"",branch:myBranches[0]||""}); } }}
...
function BranchManagerPanel({ user, onLogout }) {
  ...
  const [viewPlanUser,setViewPlanUser] = useState(null); // عرض خطة التطور للقيادي التابع (لا بطاقة التقييم)
  ...
  const cur = idps[empId]||{};
  const cost = planCost(cur);
  if(approve){
   const already = (users||[]).filter(u=>u.id!==empId && idps[u.id]?.branchFinanceApproved).reduce((s,u)=>s+planCost(idps[u.id]),0);
   if(already+cost > branchCap){
    showToast(`⚠️ يتجاوز سقف الفرع (المتبقّي ${(branchCap-already).toLocaleString("en-US")} ريال)`,"#EF4444");
    return;
   }
  }
  ...
  {tab==="growth"&&(
  <div>
   <AnalyticsDashboard scope={branchEmps} evals={evals} idps={idps} impactData={impactData} unitLabel="مرحلة" getUnit={(u)=>u.stage||"—"} fixedMode="growth"/>
  ...
  <div style={{fontSize:20,fontWeight:900,color:"#15385C",fontFamily:MONO}}>{card.v}</div>
   ...
   return(
   <details style={{background:"#F6FFFB",border:"1px solid #C9EFDD",borderRadius:16,marginBottom:14,overflow:"hidden"}}>
  ...
  {tab==="eval"&&(
  <div>
   <AnalyticsDashboard scope={branchEmps} evals={evals} idps={idps} impactData={impactData} unitLabel="مرحلة" getUnit={(u)=>u.stage||"—"} fixedMode="eval"/>
   <details style={{background:BRAND.cardBg,border:`1px solid ${BRAND.cardBorder}`,borderRadius:20,margin:"18px 0 14px",overflow:"hidden",boxShadow:"0 8px 26px rgba(46,127,184,0.10)"}}>
   ...
   // شرط الاكتمال: كل الموظفين لديهم تقييم مكتمل (كل أطرافهم المطلوبة مسجّلة → avg موجود)
   const evalReady = emps.length>0 && emps.every(u=>{ const es=getEmpFullStats(u,evals[u.id]||{}); return es?.avg!=null; });
   return(
  ...
  <div><div style={{fontSize:17,fontWeight:900,color:"#15385C",letterSpacing:"-0.3px"}}>{multiBranch?`🏛️ ${br} — `:""}📚 {stage}</div><div style={{fontSize:11,color:"#8CA3BD",display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><span>{emps.length} موظف{apEval&&info?` • اعتمدها ${info.by} • ${info.at}`:""}</span>{(()=>{ const ws=branchWindowStatus(evalWinData,br); return <span style={{color:ws.color,background:`${ws.color}12`,padding:"1px 8px",borderRadius:10,fontWeight:700}}>{ws.icon} التقييم {ws.label}</span>; })()}</div></div>
  <button onClick={()=>{ if(apEval){revokeStageEval(br,stage);} else if(evalReady){approveStageEval(br,stage);} }} disabled={!apEval&&!evalReady} title={!apEval&&!evalReady?"يلزم اكتمال تقييم جميع الموظفين أولاً":""} style={{padding:"9px 20px",borderRadius:22,border:`2px solid ${apEval?"#10B98160":(evalReady?"#F59E0B60":"#DDE9F5")}`,background:apEval?"#10B98118":(evalReady?"#F59E0B18":"#F1F5F9"),color:apEval?"#10B981":(evalReady?"#F59E0B":"#94A3B8"),fontSize:12,cursor:(apEval||evalReady)?"pointer":"not-allowed",fontWeight:800}}>{apEval?"✅ معتمدة — قراءة النتائج مفتوحة":(evalReady?"🏅 اعتماد نتائج المرحلة":"⏳ بانتظار اكتمال التقييم")}</button>
  </div>
  ...
  <button onClick={()=>setViewUser(u)} style={{padding:"4px 10px",borderRadius:7,border:"1px solid #C7DBF0",background:"#fff",color:"#5B7A9E",fontSize:10,cursor:"pointer"}}>عرض</button>
   ...
   {/* قيادات الفرع: مدراء المراحل والوكلاء (يقيّمهم مدير الفرع، منفصلين عن مجموعات المراحل) */}
   {branchLeaders.length>0&&(
   <div style={{background:BRAND.cardBg,border:`1px solid ${BRAND.cardBorder}`,borderRadius:20,marginBottom:12,padding:18,boxShadow:"0 8px 26px rgba(46,127,184,0.10)"}}>
  ...
  {tab==="approve"&&(
  <div>
   {/* ═══ الاعتماد المالي النهائي (مدير الفرع) ═══ */}
   {(()=>{
    ...
    return (
    <div style={{background:"linear-gradient(135deg,#7C3AED12,#7C3AED04)",border:"1px solid #7C3AED30",borderRadius:16,padding:16,marginBottom:16}}>
    <div style={{fontSize:14,fontWeight:900,color:"#6D28D9",marginBottom:6}}>💰 الاعتماد المالي النهائي</div>
    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:12}}>
    <div style={{background:"#fff",borderRadius:10,padding:"8px 14px",border:"1px solid #7C3AED20"}}><span style={{fontSize:10,color:"#8CA3BD"}}>سقف الفرع: </span><span style={{fontSize:14,fontWeight:900,color:"#6D28D9",fontFamily:MONO}}>{branchCap.toLocaleString("en-US")}</span></div>
  ...
  <MyPlanAndEval user={user} idps={idps} evals={evals} impactData={impactData} readings={readings} locks={locks} setLocks={setLocks} evalWinData={evalWinData}
   ...
   <div><div style={{fontSize:16,fontWeight:900,color:"#15385C"}}>📋 خطة التطوّر المهني — {viewPlanUser.name}</div><div style={{fontSize:11,color:"#8CA3BD"}}>{ROLES_LIST[viewPlanUser.role]}{viewPlanUser.stage?` • ${viewPlanUser.stage}`:""}</div></div>
   <button onClick={()=>setViewPlanUser(null)} style={{background:"none",border:"none",fontSize:22,color:"#5B7A9E",cursor:"pointer"}}>✕</button>
   </div>
   <EmployeeGrowthPlan user={viewPlanUser} empEval={evals[viewPlanUser.id]||{}} idpData={idps[viewPlanUser.id]} onSave={()=>{}} viewerRole="viewer" impactData={impactData} minHours={getEvalModel(viewPlanUser)==="leader"?15:12}/>
   </div>
   </div>
   )}
...
function LibraryManager({ onSave }) {
  ...
  // ندعم الفاصلة، الفاصلة المنقوطة، والجدولة (tab) — ومرونة في الترويسات
  const splitLine=(l)=>l.split(/[,;\t]/).map(v=>v.trim().replace(/^"|"$/g,""));
  const lines=txt.split(/\r?\n/).filter(l=>l.trim());
  // نكتشف إن كان السطر الأول ترويسة فعلاً (يحوي كلمة ترويسة معروفة)
  const HEAD_HINTS=["name","الاسم","اسم","المصدر","البرنامج","العنوان","type","النوع","method","الأسلوب","provider","الجهة","url","الرابط","hours","الساعات","cost","التكلفة","السعر"];
  const firstCells=splitLine(lines[0]).map(h=>h.trim());
  const looksLikeHeader = firstCells.some(c=>HEAD_HINTS.includes(c));
  if(looksLikeHeader){
   const headers=firstCells;
   parsed=lines.slice(1).map(line=>{ const vals=splitLine(line); const obj={}; headers.forEach((h,i)=>{obj[h]=vals[i]||"";}); return obj; });
  } else {
   // لا ترويسة — كل سطر بيانات؛ العمود الأول = الاسم، والبقية أعمدة عامة بالترتيب
   const cols=["الاسم","الجهة","الساعات","التكلفة","الرابط"];
   parsed=lines.map(line=>{ const vals=splitLine(line); const obj={}; vals.forEach((v,i)=>{obj[cols[i]||`c${i}`]=v;}); return obj; });
  }
   }
  ...
  {isValidUrl(s.url)&&<a href={s.url} target="_blank" rel="noopener noreferrer" style={{padding:"4px 9px",borderRadius:7,background:"#3B82F612",border:"1px solid #3B82F630",color:"#3B82F6",fontSize:10,textDecoration:"none",fontWeight:700}}>🔗</a>}
  ...
  <div style={{display:"flex",flexDirection:"column",gap:6,marginTop:10,maxHeight:"46vh",overflowY:"auto"}}>
   {sources.map(s=>{
   const lnk=(compMap[cn]||[]).includes(s.name);
   return(
   <div key={s.id} onClick={()=>toggleLink(cn,s.name)}
   style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,border:`1px solid ${lnk?col+"55":"#E3EEF9"}`,background:lnk?`${col}0E`:"#FBFDFF",cursor:"pointer",transition:"all 0.12s"}}>
   <div style={{width:22,height:22,borderRadius:6,border:`2px solid ${lnk?col:"#CBD5E1"}`,background:lnk?col:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,color:"#fff",fontSize:13,fontWeight:900}}>{lnk?"✓":""}</div>
   <span style={{fontSize:15,flexShrink:0}}>{s.type==="كتاب"?"📖":"🎓"}</span>
   <div style={{flex:1,minWidth:0}}>
   <div style={{fontSize:12.5,color:lnk?col:"#334155",fontWeight:lnk?700:600,lineHeight:1.5}}>{s.name}</div>
   {(s.provider||s.hours)&&<div style={{fontSize:10,color:"#8CA3BD",marginTop:1}}>{s.provider||""}{s.provider&&s.hours?" • ":""}{s.hours?`${s.hours} ساعة`:""}</div>}
   </div>
   </div>
   );
   })}
  ...
  {["داخلي حضوري أو عن بعد","خارجي حضوري أو عن بعد","إلكتروني","قراءة"].map(m=><option key={m} value={m}>{m}</option>)}
  </select>
   </div>
   <div style={{gridColumn:"1 / -1"}}>
...
// رابط صالح فقط إن بدأ بـ http/https — يمنع تحويل النصوص (مثل "يصلكم لاحقاً") لمسار داخل النظام
const isValidUrl = (u) => typeof u==="string" && /^https?:\/\/.+/i.test(u.trim());
// حالة نافذة تقييم فرع معيّن (تُستخدم في كل اللوحات القيادية لعرض/تعطيل التقييم)
// win = كائن النوافذ الكامل {branches:{br:{isOpen,openDate,closeDate}}}
const branchWindowStatus = (win, branch) => {
  const today = new Date().toISOString().split("T")[0];
  const w = (win && win.branches) ? (win.branches[branch] || {}) : {};
  const isExp = w.closeDate && w.closeDate < today;
  const isOpen = !!w.isOpen && !isExp;
  return isOpen ? {open:true, label:"مفتوح", color:"#10B981", icon:"🔓"}
       : isExp   ? {open:false, label:"انتهت المدة", color:"#8B5CF6", icon:"⌛"}
       :           {open:false, label:"مغلق", color:"#EF4444", icon:"🔒"};
};
...
function ExecEvalReport({ users, evals, approvals, locks, onOpenCard }) {
  ...
  const lvDist = {"يفوق التوقعات":0,"ممتاز":0,"جيد جداً":0,"جيد":0,"دون التوقعات":0};
  ...
  const lvColors = {"يفوق التوقعات":"#10B981","ممتاز":"#3B82F6","جيد جداً":"#F59E0B","جيد":"#F97316","دون التوقعات":"#EF4444"};
...
function DashDonut({ percent, color, label, sub, size=140 }) {
...
// mode: "growth" | "eval"
// ═══════════════════════════════════════════════════════════
function AnalyticsDashboard({ scope, evals, idps, impactData, unitLabel="فرع", getUnit, fixedMode }) {
  const [modeS,setMode] = useState(fixedMode||"growth");
  const mode = fixedMode || modeS;
  ...
  <div>
   ...
   <DashDonut percent={growthStats.pct(growthStats.planned)} color="#10B981" label="التخطيط" size={120}/>
   <DashDonut percent={growthStats.rpct(growthStats.execRows)} color="#F59E0B" label="التنفيذ" size={120}/>
   <DashDonut percent={growthStats.rpct(growthStats.measRows)} color="#8B5CF6" label="قياس الأثر" size={120}/>
   </div>
   </ChartCard>
...
function TranslationsManager({ onSave, showToast }) {
  const [dict,setDict] = useState(()=>{
   try {
    // نبدأ بكل مفاتيح النظام المستخرجة (قيم فارغة) ثم البذرة ثم المحفوظ
    const base = {};
    (I18N_KEYS||[]).forEach(k=>{ base[k]=""; });
...
function AdminPanel({ onLogout, assistant }) {
  ...
  const [form,setForm] = useState({name:"",username:"",password:"",role:"employee",job:"",branch:"",stage:"",nationalId:"",jobNumber:"",supervisorId:"",stageManagerId:"",peerId:""});
  const [toast,setToast] = useState(null);
  const [viewUser,setViewUser] = useState(null);
  ...
  const [maintMode,setMaintMode] = useState({enabled:false,message:""}); // وضع الصيانة (تعطيل الدخول)
  ...
  st.get("acctRequests_360c").then(d=>setAcctRequests(Array.isArray(d)?d:[]));
  st.getShared("maintenanceMode_360c").then(d=>{ if(d) setMaintMode({enabled:!!d.enabled,message:d.message||""}); });
  ...
  // استيراد جماعي للموظفين (معلم/إداري فقط) — لصق من Excel: الاسم | البريد | كلمة المرور | الفرع | المرحلة | المسمّى | الرقم الوظيفي | الهوية
  const [showEmpImport,setShowEmpImport] = useState(false);
  const [empImportTxt,setEmpImportTxt] = useState("");
  const [empImportReport,setEmpImportReport] = useState(null);
  const handleEmpImport = async () => {
   const txt=(empImportTxt||"").trim();
   if(!txt){ setEmpImportReport({error:"الرجاء لصق البيانات."}); return; }
  ...
  // تطبيع البريد: إزالة المسافات والمحارف الخفية (قد يُدرجها المتصفّح العربي)
  const cleanUsername = String(form.username).replace(/[\u200B-\u200F\u202A-\u202E\uFEFF]/g,"").trim();
  if (cleanUsername !== form.username) setForm(p=>({...p,username:cleanUsername}));
  // مدير النظام معفى من فرض الدومين (مسؤول)، لكن نتحقّق أن البريد بصيغة صحيحة عموماً
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(cleanUsername)) { showToast("صيغة البريد غير صحيحة","#EF4444"); return; }
  if (form.nationalId && !/^\d{10}$/.test(String(form.nationalId).trim())) { showToast("رقم الهوية يجب أن يكون 10 أرقام","#EF4444"); return; }
  if ((users||[]).find(u=>u.username===cleanUsername)) { showToast("اسم المستخدم موجود","#EF4444"); return; }
  const newUser = {...form,username:cleanUsername,id:Date.now().toString(),peerIds:form.peerIds||(form.peerId?[form.peerId]:[])};
  try {
  ...
  <div style={{display:"grid",gridTemplateColumns:"360px 1fr",gap:16}}>
   ...
   <div style={{fontSize:13,color:"#2E7FB8",fontWeight:800}}>➕ إضافة حساب</div>
   <button onClick={()=>{setShowEmpImport(true);setEmpImportReport(null);}} style={{padding:"7px 12px",borderRadius:9,border:"none",background:"linear-gradient(135deg,#059669,#10B981)",color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer"}} title="استيراد معلمين/إداريين دفعة واحدة من Excel">📥 استيراد معلمين/إداريين</button>
   </div>
   <div style={{display:"flex",flexDirection:"column",gap:9}}>
  {[{l:"الاسم *",k:"name"},{l:"اسم المستخدم *",k:"username"},{l:"كلمة المرور *",k:"password",t:"password"},{l:"🪪 رقم الهوية",k:"nationalId",ph:"رقم الهوية الوطنية"},{l:"🔢 الرقم الوظيفي",k:"jobNumber",ph:"4 أو 5 أرقام"}].map(f=>(
  <div key={f.k}>
  ...
  {[
  {l:"الصلاحية *",k:"role",opts:[{v:"",l:"-- اختر --"},...Object.entries(ROLES_LIST).filter(([k])=>!(isAssistant&&(k==="admin"||k==="admin_assistant"))).map(([v,l])=>({v,l}))]},
  ...
  <div style={{display:"flex",flexDirection:"column",gap:10}}>
   ...
   <div>
   <div style={{fontSize:14,fontWeight:900,color:maintMode.enabled?"#DC2626":"#15385C"}}>🛠️ وضع الصيانة (تعطيل الدخول)</div>
   <div style={{fontSize:11,color:"#8CA3BD",marginTop:2}}>عند التفعيل، يُمنع دخول جميع الحسابات عدا مدير النظام، مع عرض رسالة «قيد التحديث».</div>
   </div>
   <button onClick={async()=>{ const nm={...maintMode,enabled:!maintMode.enabled}; setMaintMode(nm); await st.setShared("maintenanceMode_360c",nm); showToast(nm.enabled?"🛠️ فُعّل وضع الصيانة — الدخول معطّل":"✅ أُلغي وضع الصيانة — الدخول متاح", nm.enabled?"#DC2626":"#10B981"); }}
    style={{padding:"9px 20px",borderRadius:22,border:"none",background:maintMode.enabled?"linear-gradient(135deg,#DC2626,#EF4444)":"linear-gradient(135deg,#059669,#10B981)",color:"#fff",fontSize:12,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>
    {maintMode.enabled?"⏹️ إيقاف وإتاحة الدخول":"▶️ تفعيل وضع الصيانة"}
   </button>
   </div>
   {maintMode.enabled&&(
   <div>
   <label style={{fontSize:11,color:"#B45309",fontWeight:700,display:"block",marginBottom:4}}>✍️ رسالة تظهر للمستخدمين وقت الصيانة</label>
   <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
   <input value={maintMode.message} onChange={e=>setMaintMode({...maintMode,message:e.target.value})} placeholder="مثال: النظام قيد التحديث حتى الساعة 5 مساءً، نعتذر عن الإزعاج"
    style={{flex:1,minWidth:220,padding:"9px 12px",borderRadius:10,border:"1px solid #FCA5A5",background:"#fff",fontSize:12,color:"#15385C"}}/>
   <button onClick={async()=>{ await st.setShared("maintenanceMode_360c",maintMode); showToast("✓ حُفظت رسالة الصيانة"); }}
    style={{padding:"9px 18px",borderRadius:10,border:"none",background:"#DC2626",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>💾 حفظ الرسالة</button>
   </div>
   </div>
   )}
   </div>
  ...
  />
   ...
   {showEmpImport&&(
  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:800,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
  <div style={{background:"#FFFFFF",border:"1px solid #10B98140",borderRadius:20,width:"100%",maxWidth:680,maxHeight:"90vh",overflowY:"auto",padding:24,direction:"rtl"}}>
   <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
   <span style={{fontSize:17,fontWeight:900,color:"#15385C"}}>📥 استيراد معلمين / إداريين</span>
   ...
   style={{width:"100%",padding:"10px",background:"#F4F9FE",border:"1px solid #C7DBF0",borderRadius:8,color:"#1E293B",fontFamily:"monospace",fontSize:12,boxSizing:"border-box",resize:"vertical",direction:"rtl"}}/>
   {empImportReport&&empImportReport.error&&<div style={{color:"#EF4444",fontSize:12,marginTop:8,background:"#EF444412",padding:"8px 12px",borderRadius:8}}>{empImportReport.error}</div>}
   {empImportReport&&empImportReport.ok!=null&&(
   <div style={{marginTop:10,background:"#F0FDF4",border:"1px solid #86EFAC",borderRadius:10,padding:"12px 14px",fontSize:12,color:"#15803D",lineHeight:1.9}}>
   ✅ أُنشئ <strong>{empImportReport.ok}</strong> من <strong>{empImportReport.total}</strong> حساباً.
   {empImportReport.errorsTotal>0&&(
   <div style={{color:"#B45309",marginTop:6}}>⚠️ {empImportReport.errorsTotal} سطراً لم يُستورَد:<br/>{empImportReport.errors.map((er,i)=><div key={i} style={{fontSize:11}}>• {er}</div>)}{empImportReport.errorsTotal>25?"...":""}</div>
   )}
   </div>
   )}
   <div style={{display:"flex",gap:10,marginTop:12}}>
   <button onClick={()=>{setShowEmpImport(false);setEmpImportTxt("");setEmpImportReport(null);}} style={{flex:1,padding:"11px",borderRadius:10,border:"1px solid #C7DBF0",background:"transparent",color:"#5B7A9E",cursor:"pointer",fontWeight:700}}>إغلاق</button>
   <button onClick={handleEmpImport} disabled={!empImportTxt.trim()} style={{flex:2,padding:"11px",borderRadius:10,border:"none",background:empImportTxt.trim()?"linear-gradient(135deg,#059669,#10B981)":"#DDE9F5",color:empImportTxt.trim()?"#fff":"#334155",fontWeight:700,cursor:empImportTxt.trim()?"pointer":"default"}}>📥 استيراد الحسابات</button>
   </div>
  </div>
  </div>
   )}
   ...
   <div style={{background:"#FFFFFF",border:"1px solid #B3D0EA",borderRadius:20,width:"100%",maxWidth:460,maxHeight:"90vh",overflowY:"auto",padding:28}}>
  ...
  {STAGES.map(s=><option key={s} value={s}>{s}</option>)}
   ...
   <div><label style={{display:"block",fontSize:11,color:"#5B7A9E",marginBottom:4,fontWeight:700}}>💼 المسمّى الوظيفي</label>
  <select value={editUser.job||""} onChange={e=>setEditUser(p=>({...p,job:e.target.value}))}
  style={{width:"100%",padding:"8px 10px",background:"#F4F9FE",border:"1px solid #DDE9F5",borderRadius:8,color:editUser.job?"#15385C":"#8CA3BD",fontSize:12}}>
  <option value="">-- اختر المسمّى --</option>
  {Object.keys(customJobs||getActiveJobs()).map(j=><option key={j} value={j}>{j}</option>)}
  </select>
  <div style={{fontSize:9,color:"#8CA3BD",marginTop:3}}>تغيير المسمّى يُحدّث الجدارات المرتبطة بالحساب تلقائياً.</div></div>
...
function ImpactMeasure({ row, impact, editable, onSave, planApproved, branchApproved }) {
  ...
  <span style={{flex:1,fontSize:11,color:"#15385C",wordBreak:"break-all"}}>{w.type==="رابط"&&isValidUrl(w.value)?<a href={w.value} target="_blank" rel="noreferrer" style={{color:"#2E7FB8"}}>{w.value}</a>:w.value}</span>
...
function SupervisorTeamGrowth({ myTargets, idps, evals, editRequests, approvals, impactData, onSaveImpact, user, onApprove, onSaveIdp, onRequestEdit, onOpenPlan }) {
  ...
  return(
   ...
   {isInternalCourse?(
   <span style={{fontSize:10,color:"#8B5CF6",background:"#8B5CF610",border:"1px solid #8B5CF630",borderRadius:8,padding:"5px 10px",fontWeight:700}}>⏳ يصلكم لاحقاً من التدريب — يُفضّل الانتهاء من الدورات خلال الفصل الأول</span>
   ):approved?(
  ...
  {["لم يتم التنفيذ","جاري التنفيذ","تم التنفيذ"].map(s=><option key={s} value={s}>{s}</option>)}
   ...
   <input value={r.supervisorNote||""} onChange={e=>onSaveIdp&&onSaveIdp(u.id,{...idps[u.id],plan:rows.map(x=>x.id===r.id?{...x,supervisorNote:e.target.value}:x)})}
   placeholder="اكتب توجيهاً أو ملاحظة على هذا البند التطويري..."
   style={{width:"100%",padding:"7px 9px",background:"#fff",border:"1px solid #3B82F625",borderRadius:6,color:"#1E293B",fontSize:11,boxSizing:"border-box",outline:"none"}}/>
   </div>
   </div>
  );
  })}
...
function LeaderPlanApprovals({ user, users, idps, impactData, readings, onApprovePlan, onOpenCard }) {
  ...
  <div style={{background:"#FFFFFF",border:"1px solid #8B5CF625",borderRadius:14,padding:16,marginTop:14}}>
   <div style={{fontSize:13,color:"#7C3AED",fontWeight:800,marginBottom:10}}>🏛️ قيادات الفرع — اعتماد خطط التطوّر المهني للقياديين التابعين</div>
   {pending.map(t=>{
...
function StageManagerPanel({ user, onLogout }) {
  ...
  const cost = planCost(cur);
  if(approve){
   const cap = stageMgrCap(user, users);
   const already = myTargets.filter(t=>t.id!==targetId && idps[t.id]?.financeApproved).reduce((s,t)=>s+planCost(idps[t.id]),0);
   if(already+cost > cap){
    showToast(`\u26a0\ufe0f يتجاوز سقفك المالي (المتبقّي ${(cap-already).toLocaleString("en-US")} ريال)`,"#EF4444");
    return;
   }
  }
  const ni = {...idps,[targetId]:{...cur, financeApproved:approve, financeApprovedBy:approve?user.name:null, financeApprovedAt:approve?new Date().toISOString().split("T")[0]:null, financeApprovedAmount:approve?cost:0}};
  setIdpsState(ni); await st.set("idps_360c",ni);
  showToast(approve?`\u2705 اعتماد مالي (${cost.toLocaleString("en-US")} ريال)`:"\u21a9 أُلغي الاعتماد المالي");
  };
  ...
  {[{k:"growth",l:"👥 متابعة التطور المهني",c:"#10B981"},{k:"finance",l:"💰 الاعتماد المالي",c:"#D97706"},{k:"eval",l:"📋 متابعة تقييم الأداء",c:"#3B82F6"},{k:"mine",l:"🎯 خطتي وتقييمي",c:"#8B5CF6"}].map(t=>(
  ...
  {/* ═══ تبويب الاعتماد المالي (مدير المرحلة) ═══ */}
  {tab==="finance"&&(()=>{
   const cap = stageMgrCap(user, users);
   // الموظفون الخاضعون للاعتماد المالي: معلم/إداري بمرحلته، خطتهم معتمدة فنياً
   const finTargets = myTargets.filter(t=>underStageFinance(t) && idps[t.id]?.approved);
   const usedAmount = finTargets.filter(t=>idps[t.id]?.financeApproved).reduce((s,t)=>s+planCost(idps[t.id]),0);
   const remaining = cap - usedAmount;
   return (
   <div>
   <div style={{background:"linear-gradient(135deg,#D9770618,#D9770608)",border:"1px solid #D9770630",borderRadius:16,padding:"16px 20px",marginBottom:16}}>
   <div style={{fontSize:14,fontWeight:900,color:"#B45309",marginBottom:10}}>💰 سقفك المالي للاعتماد</div>
   <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:10}}>
   <div style={{background:"#fff",borderRadius:12,padding:"12px 14px",border:"1px solid #D9770620"}}>
   <div style={{fontSize:10,color:"#8CA3BD",fontWeight:700}}>السقف الكلّي</div>
   <div style={{fontSize:20,fontWeight:900,color:"#B45309",fontFamily:MONO}}>{cap.toLocaleString("en-US")}</div>
   <div style={{fontSize:9,color:"#8CA3BD"}}>400 × {myTargets.filter(underStageFinance).length} موظف</div>
   </div>
   <div style={{background:"#fff",borderRadius:12,padding:"12px 14px",border:"1px solid #10B98120"}}>
   <div style={{fontSize:10,color:"#8CA3BD",fontWeight:700}}>المعتمَد</div>
   <div style={{fontSize:20,fontWeight:900,color:"#059669",fontFamily:MONO}}>{usedAmount.toLocaleString("en-US")}</div>
   </div>
   <div style={{background:"#fff",borderRadius:12,padding:"12px 14px",border:`1px solid ${remaining<0?"#EF444420":"#2E7FB820"}`}}>
   <div style={{fontSize:10,color:"#8CA3BD",fontWeight:700}}>المتبقّي</div>
   <div style={{fontSize:20,fontWeight:900,color:remaining<0?"#EF4444":"#2E7FB8",fontFamily:MONO}}>{remaining.toLocaleString("en-US")}</div>
   </div>
   </div>
   <div style={{marginTop:10,height:8,background:"#EEF4FB",borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:`${cap>0?Math.min(100,(usedAmount/cap)*100):0}%`,background:usedAmount>cap?"#EF4444":"#D97706",borderRadius:4}}/></div>
   <div style={{fontSize:10,color:"#8CA3BD",marginTop:6,lineHeight:1.6}}>تعتمد ماليّاً تكلفة خطة كل موظف (معلم/إداري) ضمن سقفك. ما يتجاوز السقف يبقى لمدير الفرع اعتماده. لا يشمل هذا القيادات أو المتابع الفني أو المشرف المختص (يعتمدهم مدير الفرع مباشرة).</div>
   </div>
   {finTargets.length===0?(
   <div style={{textAlign:"center",padding:36,color:"#5B7A9E",background:"#fff",borderRadius:12}}>لا توجد خطط معتمدة فنياً بانتظار اعتمادك المالي بعد.</div>
   ):finTargets.map(t=>{
   const idp=idps[t.id]||{}; const cost=planCost(idp); const fApproved=idp.financeApproved;
   const wouldExceed = !fApproved && (usedAmount+cost>cap);
   return(
   <div key={t.id} style={{background:"#fff",border:`1px solid ${fApproved?"#10B98130":wouldExceed?"#EF444430":"#E3EEF9"}`,borderRadius:12,padding:"12px 16px",marginBottom:8,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
   <div style={{flex:1,minWidth:140}}>
   <div style={{fontWeight:700,fontSize:13,color:"#15385C"}}>{t.name}</div>
   <div style={{fontSize:10,color:"#8CA3BD"}}>{t.job}{t.stage?` • ${t.stage}`:""} • {(idp.plan||[]).length} بند</div>
   </div>
   <div style={{textAlign:"center"}}>
   <div style={{fontSize:9,color:"#8CA3BD"}}>تكلفة الخطة</div>
   <div style={{fontSize:16,fontWeight:900,color:"#B45309",fontFamily:MONO}}>{cost.toLocaleString("en-US")}</div>
   </div>
   {fApproved
    ? <button onClick={()=>approveFinance(t.id,false)} style={{padding:"7px 14px",borderRadius:8,border:"1px solid #EF444430",background:"#EF444410",color:"#EF4444",fontSize:11,cursor:"pointer",fontWeight:700}}>↩ إلغاء</button>
    : <button onClick={()=>approveFinance(t.id,true)} disabled={wouldExceed} title={wouldExceed?"يتجاوز سقفك المالي":""} style={{padding:"7px 14px",borderRadius:8,border:"none",background:wouldExceed?"#CBD5E1":"linear-gradient(135deg,#D97706,#F59E0B)",color:"#fff",fontSize:11,cursor:wouldExceed?"not-allowed":"pointer",fontWeight:700}}>{wouldExceed?"🔒 يتجاوز السقف":"💰 اعتماد مالي"}</button>}
   </div>
   );
   })}
   {finTargets.some(t=>!idps[t.id]?.financeApproved&&(usedAmount+planCost(idps[t.id])>cap))&&(
   <div style={{background:"#EF444410",border:"1px solid #EF444430",borderRadius:10,padding:"10px 14px",marginTop:8,fontSize:11,color:"#B91C1C",lineHeight:1.7}}>
   ⚠️ بعض الخطط تتجاوز سقفك المالي المتبقّي. هذه ستبقى لمدير الفرع لاعتمادها ضمن سقفه الأوسع.
   </div>
   )}
   </div>
   );
  ...
  {tab==="eval"&&(
  <div>
   <AnalyticsDashboard scope={myTargets} evals={evals} idps={idps} impactData={impactData} unitLabel="مرحلة" getUnit={(u)=>u.stage||"—"} fixedMode="eval"/>
   {(()=>{
  ...
  return s+items.filter((_,i)=>(myEval[c]?.[i]||0)>0).length;
   ...
   const winSt = branchWindowStatus(evalWinData, u.branch);
   return(
  ...
  <div style={{marginTop:3}}><span style={{fontSize:9,color:winSt.color,background:`${winSt.color}12`,padding:"1px 7px",borderRadius:10,fontWeight:700}}>{winSt.icon} التقييم {winSt.label}</span></div>
  </div>
  ...
  : (!winSt.open&&!done)
  ? <div style={{padding:"6px 12px",borderRadius:8,background:"#F1F5F9",border:"1px solid #E2E8F0",color:"#94A3B8",fontSize:11,fontWeight:700}} title="نافذة تقييم الفرع مغلقة">🔒 التقييم مغلق</div>
...
<LogoImg style={{height:32}} size={15}/>
  ...
  {supTab==="my_growth"&&(
  <EmployeeGrowthPlan user={user} empEval={myEmpEval} idpData={idps[user.id]} onSave={saveMyIdp} viewerRole="employee" impactData={impactData} minHours={getEvalModel(user)==="leader"?15:12}/>
  )}
  ...
  // ب-3: حالة نافذة تقييم فرع هذا الموظف
  ...
  <span style={{fontSize:9,color:winSt.color,background:`${winSt.color}12`,padding:"1px 7px",borderRadius:10,fontWeight:700}}>{winSt.icon} التقييم {winSt.label}</span>
  ...
  // ب-3: لا يُسمح ببدء/تعديل التقييم إذا كانت نافذة فرع الموظف مغلقة (إلا العرض)
  if (!winSt.open && !done) return <div style={{padding:"6px 12px",borderRadius:8,background:"#F1F5F9",border:"1px solid #E2E8F0",color:"#94A3B8",fontSize:11,fontWeight:700}} title="لا يمكن التقييم — نافذة تقييم الفرع مغلقة">🔒 التقييم مغلق</div>;
  return (
  <button onClick={()=>setEvalTarget(u)}
  title={noComps?"لا جدارات عامة/فنية لهذا المسمى — راجع مصفوفة الجدارات":(!winSt.open?"النافذة مغلقة — عرض فقط":"تقييم الجدارات العامة والفنية")}
...
function EmployeeGrowthPlan({ user, empEval, idpData, onSave, viewerRole, impactData, minHours=12 }) {
  ...
  const planReadiness = (() => {
   const cats = ["أساسية","عامة","فنية"];
   // الفئة: من r.cat، أو من فئة الجدارة، أو من الجدارة المرتبطة بالمصدر
   const rowCat = (r) => r.cat || (r.comp ? getCat(r.comp) : null);
   const coveredCats = new Set(idpPlan.map(rowCat).filter(Boolean));
   // إن لم تُحدَّد فئات إطلاقاً (خطة من المخطّط المبسّط بلا cat)، لا نُلزم تغطية الفئات الثلاث
   const anyCat = idpPlan.some(r => rowCat(r));
   const missingCats = anyCat ? cats.filter(c => !coveredCats.has(c)) : [];
   // وضع الصف: صريح (auto/manual) أو مُستنتَج — وجود comp/programName ⇒ تلقائي (من المكتبة)
   const isAuto = (r) => r.mode ? r.mode==="auto" : !!((r.comp&&String(r.comp).trim())||(r.programName&&String(r.programName).trim()));
   const rowIncomplete = (r) => {
    const auto = isAuto(r);
    const need = auto
     ? ["programName","targetDate","evalMethod"]
     : ["needSource","trainMethod","programName","targetDate","evalMethod"];
  ...
  <div style={{fontSize:10,color:"#5B7A9E",marginTop:2}}>{role==="supervisor"?(idpData?.isFinal?"راجع بنود الخطة ثم اعتمدها":"الخطة في مرحلة الحفظ المؤقت — تُعتمد بعد أن يحفظها الموظف ويغلق التخطيط"):"يمكنك التعديل حتى يعتمدها المتابع الفني"}</div>
   ...
   {canApprove&&idpPlan.length>0&&!idpData?.isFinal&&<span style={{padding:"7px 14px",borderRadius:20,background:"#F1F5F9",color:"#94A3B8",fontSize:11,fontWeight:700}}>⏳ بانتظار إغلاق الموظف للتخطيط</span>}
   </div>
  )}
  ...
  {/* ملاحظة المتابع الفني على هذا البند (تظهر للموظف ليراعيها) */}
  {row.supervisorNote&&row.supervisorNote.trim()&&(
  <div style={{background:"#3B82F60D",border:"1px solid #3B82F630",borderRadius:10,padding:"9px 12px",marginBottom:12}}>
  <div style={{fontSize:10,color:"#3B82F6",fontWeight:800,marginBottom:3}}>📝 ملاحظة المتابع الفني</div>
  <div style={{fontSize:12,color:"#15385C",lineHeight:1.6}}>{row.supervisorNote}</div>
  </div>
  )}
  ...
  {isValidUrl(F("url"))
   ? <a href={F("url")} target="_blank" rel="noopener noreferrer" style={{fontSize:10,color:"#3B82F6",background:"#3B82F612",padding:"3px 10px",borderRadius:20,textDecoration:"none",fontWeight:700}}>🔗 فتح الرابط</a>
   : F("url")&&<span style={{fontSize:10,color:"#8B5CF6",background:"#8B5CF610",padding:"3px 10px",borderRadius:20,fontWeight:700}}>{F("url")}</span>}
   </div>
   {/* عدد الساعات: قابل للإدخال إن لم تأتِ من المكتبة (يلزم لإكمال التخطيط) */}
   ...
   {isValidUrl(F("url"))&&<a href={F("url")} target="_blank" rel="noopener noreferrer" style={{padding:"8px 10px",borderRadius:8,background:"#3B82F615",color:"#3B82F6",fontSize:12,textDecoration:"none",display:"flex",alignItems:"center"}}>🔗</a>}
   </div>
   </div>
   <div>
   <label style={lS}>💰 التكلفة</label>
  ...
  <div style={{background:"linear-gradient(135deg,#8B5CF60D,#6D28D908)",border:"1px solid #8B5CF630",borderRadius:14,padding:16,marginTop:14}}>
   ...
   {selCert.cost&&<span>💰 التكلفة: {selCert.cost}</span>}{selCert.cost&&selCert.url&&<span> • </span>}{isValidUrl(selCert.url)&&<a href={selCert.url} target="_blank" rel="noopener" style={{color:"#7C3AED"}}>🔗 تفاصيل الشهادة</a>}
   </div>
   )}
  ...
  {idpPlan.length>0&&canEditFields&&!approved&&(
...
function ExecPanel({ user, onLogout }) {
  ...
  const [approvals,setApprovals] = useState({});
  const [viewTarget,setViewTarget] = useState(null);
  const [viewPlanTarget,setViewPlanTarget] = useState(null);
  const [evalWinData,setEvalWinData] = useState({branches:{}});
  const [toast,setToast] = useState(null);
  const showToast = (msg,c="#10B981") => { setToast({msg,c}); setTimeout(()=>setToast(null),2000); };
  ...
  <div style={{minHeight:"100vh",background:APP_BG,direction:"rtl",fontFamily:"'El Messiri',sans-serif"}}>
   ...
   <AnalyticsDashboard
    scope={scope}
    evals={evals} idps={idps} impactData={impactData}
    unitLabel="فرع"
    getUnit={(u)=>u.branch||"—"}
   ...
   {viewTarget&&<Card360 targetUser={viewTarget} empEval={evals[viewTarget.id]||{}} idpData={idps[viewTarget.id]} readings={readings} onSaveReadings={async d=>{setReadings(d);await st.set("readings_360c",d);}} currentUser={user} allEvals={evals} allUsers={users} approvals={approvals} onClose={()=>setViewTarget(null)}/>}
   {viewPlanTarget&&(
   <div onClick={()=>setViewPlanTarget(null)} style={{position:"fixed",inset:0,background:"rgba(15,56,92,0.55)",zIndex:1000,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"30px 16px",overflowY:"auto"}}>
   <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:820,padding:24,direction:"rtl"}}>
   <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
   <div><div style={{fontSize:16,fontWeight:900,color:"#15385C"}}>🎯 خطة التطور المهني — {viewPlanTarget.name}</div>
   <div style={{fontSize:11,color:"#8CA3BD"}}>{ROLES_LIST[viewPlanTarget.role]}{viewPlanTarget.branch?` • ${viewPlanTarget.branch}`:""}</div></div>
   <button onClick={()=>setViewPlanTarget(null)} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:"#5B7A9E"}}>✕</button>
   </div>
   <EmployeeGrowthPlan user={viewPlanTarget} empEval={evals[viewPlanTarget.id]||{}} idpData={idps[viewPlanTarget.id]} onSave={()=>{}} viewerRole="exec" impactData={impactData}/>
   </div>
   </div>
   )}
...
function EmployeePanel({ user, onLogout }) {
  ...
  // علاقة الزملاء متبادلة: أُقيّم مَن أُدرِج في قائمتهم (peerIds تحويني)، ومَن أدرجتُه أنا في قائمتي.
  // هذا يضمن ظهور التقييم بصرف النظر عن اتّجاه التعيين الذي أجراه المتابع الفني.
  const myPeerIds = (user.peerIds||(user.peerId?[user.peerId]:[]));
  const peerTargets = (users||[]).filter(u=>u.id!==user.id && (
    (u.peerIds||(u.peerId?[u.peerId]:[])).includes(user.id) || myPeerIds.includes(u.id)
  ));
  // الرؤساء الذين يقيّمهم المستخدم الحالي كمرؤوس (التقييم الصاعد)
  // الجذر السابق: كان يستخدم getEvaluators (مرؤوسي القيادي) بدل رؤساء المستخدم.
  // الصحيح: رئيسي المباشر = من يعتمد خطتي في الهرمية (getPlanApprover)، ونضيف من تربطني به علاقة إشرافية مباشرة.
  const bossTargets = (()=>{
    const set = {};
    // 1) الرئيس المباشر الهرمي (مدير المرحلة→مدير الفرع→الشؤون→الرئيس التنفيذي...)
    const approver = getPlanApprover(user, users);
    if (approver && approver.id!==user.id && getEvalModel(approver)==="leader") set[approver.id]=approver;
    // 2) المتابع الفني/المدير المباشر المعيّن صراحةً إن كان قياديّاً
    (users||[]).forEach(t=>{
      if (t.id===user.id) return;
      if (getEvalModel(t)!=="leader") return;
      // إن كان هذا القيادي رئيساً مباشراً لي عبر الهرمية العكسية (أنا من مرؤوسيه المباشرين)
      const myBossViaApproval = getPlanApprover(user, users);
      if (myBossViaApproval && myBossViaApproval.id===t.id) set[t.id]=t;
    });
    return Object.values(set);
  })();
  // مقدّمو الخدمة الذين يقيّمهم المستخدم كمستفيد
  // الجذر نفسه: نستخدم العلاقة الصحيحة — المستفيد يقيّم مقدّم الخدمة الذي يخدم نطاقه.
  const serviceTargets = (users||[]).filter(t=>{
    if (t.id===user.id) return false;
    const m = getEvalModel(t);
    if (m!=="branch_ext" && m!=="specialist") return false;  // من لهم مستفيدون
    // المستفيدون من الامتداد الفني/الأخصائي = القياديون في نطاقه (getEvaluators يُعدّدهم)
  ...
  const today = new Date().toISOString().split("T")[0];
  const w = evalWindowData;
  const winOpen = w.isOpen && !(w.closeDate && w.closeDate < today);
  return (
  <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
  {(()=>{
  ...
  ? <span style={{padding:"6px 12px",borderRadius:8,background:"#F1F5F9",border:"1px solid #E2E8F0",color:"#94A3B8",fontSize:11,fontWeight:700}} title="نافذة تقييم فرع الزميل مغلقة">🔒 مغلق</span>
  : <button onClick={()=>ws.open&&setPeerTarget(t)} disabled={!ws.open}
  style={{padding:"6px 14px",borderRadius:8,border:done?"1px solid #8B5CF630":"none",background:done?"#8B5CF615":ws.open?"linear-gradient(135deg,#6D28D9,#8B5CF6)":"#CBD5E1",color:done?"#8B5CF6":"#fff",fontSize:11,cursor:ws.open?"pointer":"not-allowed",fontWeight:700}}>
  {done?"تعديل":"تقييم ▶"}
  </button>;
  })()}
  </div>
  ...
  <div style={{background:"#FFFFFF",border:"1px solid #0891B225",borderRadius:14,padding:16,marginTop:12}}>
   ...
   {(()=>{ const ws=branchWindowStatus(evalWinAll,t.branch); return (!ws.open&&!done)
   ? <span style={{padding:"6px 12px",borderRadius:8,background:"#F1F5F9",border:"1px solid #E2E8F0",color:"#94A3B8",fontSize:11,fontWeight:700}} title="نافذة تقييم فرع هذا الشخص مغلقة">🔒 مغلق</span>
   : <button onClick={()=>ws.open&&setRoleEvalTarget({user:t,party:"beneficiary"})} disabled={!ws.open} style={{padding:"6px 14px",borderRadius:8,border:done?"1px solid #0891B230":"none",background:done?"#0891B215":ws.open?"linear-gradient(135deg,#0E7490,#0891B2)":"#CBD5E1",color:done?"#0891B2":"#fff",fontSize:11,cursor:ws.open?"pointer":"not-allowed",fontWeight:700}}>{done?"تعديل":"تقييم ▶"}</button>;
   })()}
   </div>
   );
   })}
